...
// of candies among them.Notice that multiple kids can have the greatest number of
// candies.
...
var kidsWithCandies = function (candies, extraCandies) {
  ...
  for (let j = 0; j < candies.length; j++) {
    let greatestCandies = candies[j] + extraCandies
    if (greatestCandies >= mostCandies) {
      result.push(true);
    } else if (greatestCandies < mostCandies) {
      result.push(false);
    }
  }
  return result
